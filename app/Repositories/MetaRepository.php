...
class MetaRepository extends BaseRepository{
	 ...
	 */
	public function index($type = 'category', $data = [], $size = null)
	{
	 ...
	 * @return Douyasi\Models\Meta
	 */
	public function store($type = 'category', $inputs)
	{
		if($type === 'category'){
			$meta = new $this->model;
	 ...
	 * @return void
	 */
	public function update($type = 'category', $inputs, $id)
	{
		if($type === 'category'){
			$meta = $this->model->category()->findOrFail($id);
			$meta = $this->saveCategory($meta, $inputs);
	 ...
	 * @return void
	 */
	public function destroy($type = 'category', $id)
	{
		if($type === 'tag'){
			$meta = $this->model->tag()->findOrFail($id);
		}
