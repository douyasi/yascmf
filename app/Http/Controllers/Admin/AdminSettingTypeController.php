...
/**
	...
	public function index(Request $request)
	{
		...
		$types = $this->setting->index('', $data);  //注意，第一个参数为空，取的是SettingType模型
	...
	public function store(SettingTypeRequest $request)
	{
		if ($request->is_ajax())
		{
			...
			if($validator->passes()){
				$setting_type = $this->setting->store($data, '');  //注意，第二个参数为空，取的是SettingType模型
				if($setting_type->id){
	...
	public function update(SettingTypeRequest $request, $id)
	{
		if ( $request->is_ajax() && $request->is_method('put') )
		{
			...
			if($validator->passes()){
				$setting_type = $this->setting->update($data, $id, '');  //注意，第三个参数为空，取的是SettingType模型
				$json = array_replace($json, ['status' => 1, 'info' => '成功']);
			} else {
				$json = format_json_message($validator->messages(), $json);
			}
		...
		if( $request->is_ajax() && $request->is_method('delete') )
		{
			...
			if($id == 1){
				...
				if($this->setting->hasSetting($id)){
					...
					$this->setting->destroy($id, '');  //注意，第二个参数为空，取的是SettingType模型
					$json = array_replace($json, ['status'=>1, 'info' => '成功']);
				}
			}
			return response()->json($json);
