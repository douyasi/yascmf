...
class AdminPageController extends BackController {
	...
	public function index(Request $request)
	{
		...
		$pages = $this->content->index('page', $data);
	...
	public function store(PageRequest $request)
	{
		if($request->is_ajax())
		{
			...
			if($validator->passes()){
				$content = $this->content->store('page', $data, user('id'));
				if($content->id){
	...
	{
		//
		$page = $this->content->edit('page', $id);
	...
	public function update(PageRequest $request, $id)
	{
		...
		if ( $request->is_ajax() && $request->is_method('put') )
		{
			...
			if($validator->passes()){
				$this->content->update('page', $data , $id);
				$json = array_replace($json, ['status' => 1, 'info' => '成功']);
			} else {
				$json = format_json_message($validator->messages(), $json);
			}
